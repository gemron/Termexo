...
const hero = html.slice(html.indexOf('<section class="hero'), html.indexOf('<section class="signal-strip'));
const dictionaryContext = vm.createContext({});
vm.runInContext(app.slice(0, app.indexOf('const languageButtons')), dictionaryContext);
...
test('desktop and phone workflow have complete bilingual copy and honest boundaries', () => {
  ...
  const keys = [...hero.matchAll(/data-i18n="([^"]+)"/g)].map((match) => match[1]);
  for (const lang of ['en', 'zh']) {
    for (const key of keys) assert.ok(dictionaries[lang][key]?.trim(), `${lang}: ${key}`);
    assert.match(dictionaries[lang].heroRemoteNote, /VPN/);
    assert.match(dictionaries[lang].heroRemoteNote, /token|令牌/);
    assert.match(dictionaries[lang].heroRemoteNote, /PC running|电脑需保持运行/);
    assert.match(dictionaries[lang].sceneCaption, /not a live session|非实时会话/);
  }
  assert.match(dictionaries.zh.heroLine2, /手机/);
  assert.match(dictionaries.en.heroLine2, /phone/);
  assert.doesNotMatch(hero, /termexo agent status|7 agents active|Nothing leaves your PC/);
  const scene = hero.slice(hero.indexOf('<figure'));
...
test('remote guide follows language changes and preserves the remote chapter', () => {
  ...
  const context = vm.createContext({
    document: {
      ...
      querySelectorAll: (selector) => selector === '[data-i18n]' ? [link] : [],
    },
  ...
  for (const lang of ['zh', 'en', 'zh']) {
    ...
    assert.equal(link.attrs.href, lang === 'zh' ? 'guide.html#remote' : 'guide.en.html#remote');
    assert.equal(link.textContent, dictionaries[lang].heroRemoteGuide);
    const guide = readFileSync(new URL(link.attrs.href.split('#')[0], site), 'utf8');
...
test('static homepage links resolve and existing social artwork is preserved', () => {
  ...
  assert.match(html, /href="guide.en.html#remote" data-i18n="heroRemoteGuide"/);
  assert.match(html, /property="og:image"\s+content="https:\/\/www.termexo.com\/assets\/termexo-phone.png"/);
