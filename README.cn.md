...
<picture>
  ...
  <img src="website/assets/termexo-workflow-zh.svg" alt="Termexo 工作流程：项目与任务进入本地工作台，统一管理 Agent、模型和会话，通过可选远程连接跨设备继续操作">
</picture>

## 个人与企业的使用场景

| 场景 | 个人开发者 | 企业研发团队成员 | Termexo 的解决方式 |
| --- | --- | --- | --- |
| 多项目并行 | 同时维护主业项目、个人产品和开源仓库 | 按业务项目组织各自的开发现场 | 工作区集中管理项目目录、终端、布局和配置，减少来回切换。 |
| 多 Agent 分工与进度跟踪 | 分别让 Agent 写功能、补测试、审查代码 | 由工程师分配任务、查看进度并确认结果 | 并排终端、任务看板、运行状态和等待确认提醒，减少反复查看输出。 |
| 中断后继续与任务交接 | 隔天恢复会话，或换一个 Agent 接着做 | 整理目标、进度和下一步，辅助工程师之间的任务交接 | 搜索并恢复支持的原生会话，使用交接包传递任务背景。 |
| 长任务与远程操作 | 外出后用手机看进度、回复确认、发送指令 | 任务留在研发电脑运行，通过受控入口远程接入 | 自建中继或可信局域网 / VPN，连接同一个运行中的终端；宿主电脑需保持运行。 |
| 多模型与账号配置 | 管理订阅和 API 配置，按任务选择模型 | 管理不同供应商、兼容网关和网络环境的配置 | 模型、账号和网络 Profile 集中管理，查看支持的供应商余量。 |
| 自主管理开发环境 | 项目与 Agent 运行在自己的电脑，自建远程入口 | 在企业控制的研发环境中运行工作台，自行管理中继 | 本地保存工作台状态，按需启用远程访问；模型请求的去向由 Agent 和模型服务配置决定。 |

**当前能力与规划：** 上述场景以个人和团队成员使用各自的工作台为基础，由人分配任务和处理确认；多人共享、细粒度权限、统一审计及更完整的自动编排属于[长期产品规划](Termexo.md)，尚不是现成的企业管理平台能力。
...
**[v0.10.2](https://github.com/gemron/Termexo/releases/tag/v0.10.2)** 让四个 Agent 终端的状态真正可靠——用真实的 Claude Code、Codex CLI、OpenCode 和 Antigravity 会话逐一端到端验证——新增异常与限流通知，任务看板也支持全部七种语言。
...
| V0.10.2 | 可靠的 Agent 状态与通知，以及多语言任务看板 | 当前版本 |
